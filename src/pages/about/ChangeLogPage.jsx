...
const ChangeLogPage = () => {
  ...
  const Log = (props) => 
  <div className="rounded-lg border border-transparent m-5 p-2 bg-neutral-400 dark:bg-neutral-600 text-left">
    {props.title && <h3 className="py-3 font-bold text-xl">{props.title}</h3>}
    {props.message}
        ...
        for await (const response of octokit.paginate.iterator(octokit.rest.repos.listCommits, {
          ...
          for (const message of response.data) {
            ...
            let committitle = 
              ...
              commitmessage
            let commitmessagebody = commitmessage.substring(committitleend).trim().split('\n')
            promisedCommitList.push(
              <Log
                title={committitle}
                message={(committitleend != -1) && <ul>{commitmessagebody.map((value, index) => (
    ...
    <>
      ...
      <Log
        message={<ul>
          ...
          <li className="text-green-400">React Project Created: Page Routing and first few pages created</li>
  ...
  return (
    <>
      ...
      <button onClick={toggleWrittenList} className="rounded-lg border border-transparent py-2 px-4 font-medium text-white bg-gray-900 cursor-pointer transition-colors duration-200 hover:text-indigo-500 hover:border-indigo-500 focus:outline-none focus:ring-4 focus:ring-indigo-500">
        {writtenListOn ? "Written Log" : "Github Commits"}
      </button>
      <ul className="border border-gray-400 rounded p-1 m-5">
        {writtenListOn ?
        <WrittenList /> : 
        <GitHubList />}
      </ul>
